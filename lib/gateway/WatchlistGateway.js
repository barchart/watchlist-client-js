...
module.exports = (() => {
	...
	class WatchlistGateway extends Disposable {
		constructor(protocol, host, port, environment, webSocketProtocol, webSocketHost) {
			...
			this._watchlistsReadEndpoint = EndpointBuilder.for('read-watchlists', 'read your watchlists')
				...
				.withQueryBuilder((qb) =>
					qb.withVariableParameter('meta', 'meta', 'meta', true, (value) => is.boolean(value) && value ? 'true' : 'false')
				)
			...
			this._watchlistReadEndpoint = EndpointBuilder.for('read-watchlista', 'read a watchlist')
				.withVerb(VerbType.GET)
				.withProtocol(protocolType)
				.withHost(host)
				.withPort(port)
		 ...
		 *
		 * @public
		 * @param {Boolean=} meta - If true, only watchlist metadata will be returned (i.e. no "entries" will be included).
		 * @returns {Promise<Schema.Watchlist[]>}
		 */
		readWatchlists(meta) {
			return Promise.resolve()
				.then(() => {
					assert.argumentIsOptional(meta, 'meta', Boolean);
					...
					return Gateway.invoke(this._watchlistsReadEndpoint, payload);
				});
		}
		 ...
		 *
		 * @public
		 * @param {String} id - The identifier of the watchlist to read.
		 * @param {Boolean=} meta - If true, only watchlist metadata will be returned (i.e. no "entries" will be included).
		 * @returns {Promise<Schema.Watchlist[]>}
		 */
		readWatchlist(id, meta) {
			return Promise.resolve()
				.then(() => {
					...
					return Gateway.invoke(this._watchlistReadEndpoint, payload);
				});
		}
